...
import { getDrugList, startOrStopDrug, deleteDrugById } from '@/api/drug'
...
const handleEditClick = (row) => {
  router.push({ name: 'EditDrug', params: { id: row.id } })
}

// 启用/禁用功能
const startOrStop = async (row) => {
  await startOrStopDrug(row.id, row.status === 1 ? 0 : 1)
    .then((res) => {
      if (res.data.code === 1) {
        ...
        <el-table :data="tableData" style="width: 100%">
          ...
          <el-table-column fixed="right" label="操作" width="200">
            <template #default="{ row }">
              ...
              <el-button link type="danger" @click="deleteDrug(row)">删除</el-button>
            </template>
          </el-table-column>
        </el-table>
      </el-scrollbar>
