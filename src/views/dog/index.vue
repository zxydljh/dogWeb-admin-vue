...
import { getDogList, startOrStopDog, deleteDogById } from '@/api/dog'
...
const handleEditClick = (row) => {
  router.push({ name: 'EditDog', params: { id: row.id } })
}

// 启用/禁用功能
const startOrStop = async (row) => {
  await startOrStopDog(row.id, row.status === 1 ? 0 : 1)
    .then((res) => {
      if (res.data.code === 1) {
    ...
    <div>
      <el-scrollbar height="470px">
        <el-table :data="tableData" style="width: 100%">
          ...
          <el-table-column prop="blood" label="血统" width="150"/>
          ...
          <el-table-column fixed="right" label="操作" width="200">
            <template #default="{ row }">
              ...
              <el-button link type="danger" @click="deleteDog(row)">删除</el-button>
            </template>
          </el-table-column>
        </el-table>
      </el-scrollbar>
