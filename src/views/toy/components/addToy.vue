...
import { addToy, updateToy, getToyById } from '@/api/toy'
...
const fetchToyDetails = async (id) => {
  try {
    const response = await getToyById(id)
...
onMounted(() => {
  if (route.params.id) {
    ...
    fetchToyDetails(route.params.id)
  }
})
...
const onSubmit = () => {
  if (isEditMode.value) {
    // 更新食品数据
    updateToy(route.params.id, form)
      .then((res) => {
        if (res.data.code === 1) {
...
<template>
  <div class="add-commodity">
    <el-form :model="form" label-width="auto" style="max-width: 500px">
      <el-form-item label="玩具名称" required>
