...
import { getFoodList, startOrStopFood, deleteFoodById } from '@/api/food'
...
const handleEditClick = (row) => {
  router.push({ name: 'EditFood', params: { id: row.id } })
}

// 启用/禁用功能
const startOrStop = async (row) => {
  await startOrStopFood(row.id, row.status === 1 ? 0 : 1)
    .then((res) => {
      if (res.data.code === 1) {
...
<template>
  <div class="commodity-container">
    <div class="commodity-header">
      食品名称：
      <el-input v-model="input" style="width: 240px" placeholder="请输入食品名称" class="commodity-search"/>
      <el-button type="warning" @click="searchHandle">查询</el-button>
      <el-button type="primary" :icon="Plus" @click="addFood">添加</el-button>
    </div>
